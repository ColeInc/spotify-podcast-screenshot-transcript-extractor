...
**Key Features:**
1. Upload screenshots → Extract title/timestamp via Claude Vision API
2. Aggregate screenshots by video/podcast title
3. Automate playback in Spotify/YouTube using Puppeteer
4. Record screen at each timestamp (±1 minute)
5. Extract audio and transcribe using OpenAI Whisper
6. Generate markdown transcript files per video
...
4. Create `.env` with required keys:
   ...
   SPOTIFY_PASSWORD=
   ```
5. Set up file storage directories (`/uploads`, `/outputs`)
...
**Tasks:**
1. Build `upload-zone.tsx` component with drag-drop (shadcn/ui)
2. Implement `POST /api/upload` route:
   - Save files to `/uploads/screenshots/`
   - Create Screenshot records in database
3. Create `lib/claude-client.ts`:
...
4. Implement `POST /api/analyze` route:
   - Read screenshot file
   - Call Claude API
   - Parse response and update Screenshot record
5. Build `screenshot-card.tsx` to display uploads with extracted data
6. Implement batch processing with progress indicators
...
- [lib/claude-client.ts](lib/claude-client.ts)
- [app/components/upload-zone.tsx](app/components/upload-zone.tsx)
...
**Tasks:**
1. Implement `GET /api/timestamps` route:
   - Query database for all screenshots
   - Group by `videoTitle`
   - Return: `{ "Video Title": [123, 456, 789], ... }`
2. Create Video records when screenshots are analyzed
3. Build `timestamp-table.tsx` component:
   - Display video titles
   - Show all timestamps per video
   - Allow editing/deletion
4. Add manual correction UI for misidentified data
...
**Critical Files:**
- [app/api/timestamps/route.ts](app/api/timestamps/route.ts)
- [app/components/timestamp-table.tsx](app/components/timestamp-table.tsx)
...
**Tasks:**
1. Create `lib/puppeteer-automation.ts`:
...
4. Build `POST /api/automation/run` route:
...
**Critical Files:**
- [lib/puppeteer-automation.ts](lib/puppeteer-automation.ts)
- [lib/screen-recorder.ts](lib/screen-recorder.ts)
- [app/api/automation/run/route.ts](app/api/automation/run/route.ts)
...
**Tasks:**
1. Extend `lib/puppeteer-automation.ts`:
...
2. Test with various YouTube video types (podcasts, interviews, etc.)
3. Update `automation-panel.tsx` to show platform-specific status
...
**Critical Files:**
- [lib/puppeteer-automation.ts](lib/puppeteer-automation.ts) (updated)
...
1. Install ffmpeg:
   ...
   ```
2. Create `lib/audio-extractor.ts`:
   ```typescript
...
**Critical Files:**
- [lib/audio-extractor.ts](lib/audio-extractor.ts)
...
**Tasks:**
1. Create `lib/whisper-client.ts`:
   ```typescript
   ...
   ```
2. Implement `POST /api/transcribe` route:
   - Accept Recording ID
   - Read audio file
   - Send to Whisper API
   - Store transcript text in database
3. Handle large audio files (Whisper has 25MB limit):
...
**Critical Files:**
- [lib/whisper-client.ts](lib/whisper-client.ts)
...
**Tasks:**
1. Create markdown formatting utility in `lib/utils.ts`:
   ```typescript
...
**Critical Files:**
- [app/api/transcripts/generate/route.ts](app/api/transcripts/generate/route.ts)
- [app/components/transcript-viewer.tsx](app/components/transcript-viewer.tsx)
...
2. Toast notifications (shadcn/ui Toast):
   - Success messages
   - Error alerts
   - Info notifications
3. Error handling:
...
5. Dark mode support (shadcn/ui default)
...
3. Test transcription:
   ...
   - Multiple speakers
4. Write PRD.md (see outline below)
5. Write IMPLEMENTATION.md (see outline below)
...
1. **[prisma/schema.prisma](prisma/schema.prisma)** - Database schema with Screenshot, Video, Recording, Transcript, AutomationJob models

2. **[lib/claude-client.ts](lib/claude-client.ts)** - Claude Vision API wrapper for screenshot analysis

3. **[lib/whisper-client.ts](lib/whisper-client.ts)** - OpenAI Whisper API wrapper for transcription

4. **[lib/puppeteer-automation.ts](lib/puppeteer-automation.ts)** - Core automation logic for Spotify and YouTube

5. **[lib/screen-recorder.ts](lib/screen-recorder.ts)** - Screen recording using puppeteer-screen-recorder

6. **[lib/audio-extractor.ts](lib/audio-extractor.ts)** - Audio extraction using ffmpeg

7. **[app/api/upload/route.ts](app/api/upload/route.ts)** - Screenshot upload endpoint

8. **[app/api/analyze/route.ts](app/api/analyze/route.ts)** - Claude Vision API integration endpoint

9. **[app/api/automation/run/route.ts](app/api/automation/run/route.ts)** - Puppeteer automation trigger

10. **[app/api/transcribe/route.ts](app/api/transcribe/route.ts)** - Whisper transcription endpoint

11. **[PRD.md](PRD.md)** - Product Requirements Document (see structure below)

12. **[IMPLEMENTATION.md](IMPLEMENTATION.md)** - Technical Implementation Guide (see structure below)
